...
capture log close
log using "$log\C5_organize_data_2", replace text
set more off
...
mvdecode ftotval inctot incwage , mv(-9999 -99999)
...
bysort statefip (year): keep if _n == 1	
...
** merge
use "$workdata\state_wage_2015.dta",replace
merge 1:1 statefip year using "$workdata\state_wage_2016.dta"
tab _merge
drop _merge
...
use "$rawdata\cps_2014_16.dta",replace
order *, alphabet
order incwage statefip year, first
